...
class ConfigWindow:
    def __init__(self, parent, config, reload_callback):
        ...
        self._add_entry(
            general_frame, "language", "Language Code (e.g., en, de, auto):", 1
        )
        self._add_entry(
            general_frame,
            "silence_timeout",
            "Silence Timeout (s, 0=disable):",
            2,
        ...
        self._add_combobox(
            whisper_frame, "model_size", "Model Size:", models, 0, section="Whisper"
        )
        self._add_combobox(
            whisper_frame, "device", "Device:", ["cpu", "cuda"], 1, section="Whisper"
        )
        ...
        self._add_checkbutton(
            whisper_frame,
            "use_vad_filter",
            "Use VAD Filter (improves silence handling):",
            3,
            section="Whisper",
        )
        self._add_entry(
        ...
        self._add_entry(
            advanced_frame,
            "audio_device",
            "Audio Device (name or index, blank=default):",
            0,
            section="Advanced",
        )
    ...
    def _add_combobox(
        self, frame, key, label_text, values, row, section="General", type_converter=str
    ):
        self._add_widget(frame, key, label_text, row, section)
        var = tk.StringVar()
    ...
    def load_initial_values(self):
        for section, keys in self.vars.items():
            for key, (var, type_converter) in keys.items():
                value = config_manager.get_setting(
                    self.config, section, key, type_converter
                )
    ...
    def save_settings(self):
        try:
                ...
                for key, (var, type_converter) in keys.items():
                    ...
                    try:
                        ...
                        messagebox.showerror(
                            "Invalid Input",
                            f"Invalid value for '{key}': '{value}'. Please enter a valid {type_converter.__name__}.",
                        )
                        return  # Stop saving
            ...
            messagebox.showinfo(
                "Settings Saved",
                "Settings saved successfully. Some changes may require restarting the application or dictation.",
                parent=self.window,
            )
            self.window.destroy()
            ...
            messagebox.showerror(
                "Error Saving", f"Failed to save settings: {e}", parent=self.window
            )
